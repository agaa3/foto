...
bool Triangle::hit(const Ray& ray, Vector3& intPoint, Vector3& normal, float& t, float t_min, float t_max) const {
	...
	if (t1 < FLT_EPSILON) { // tu patrzymy czy trójkąt jest ZA poczatkiem promienia
		return false;
	}
...
bool Triangle::hit(const Ray& ray, float t_min, float t_max) const {
	Vector3 edge1 = vertices[1] - vertices[0];
	Vector3 edge2 = vertices[2] - vertices[0];
	Vector3 h = ray.direction.cross(edge2);
	float a = edge1.dotProduct(h);
	...
	Vector3 intPoint = ray.origin + ray.direction * t1;
